...
## Дополнительные секции если есть repo-*.md файлы

Если в папке есть файлы `repo-<namespace>.md` — добавить в HTML:

### Секция: Pod → Repo → CD цепочки

Таблица по всем сервисам кластера:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>payment-api</td>
      <td>D-01.015.11</td>
      <td><a href="...">universal-task</a></td>
      <td>cd-configs/ns-payments/...</td>
      <td>✅</td>
    </tr>
    <!-- строка красная если версии расходятся -->
  </tbody>
</table>
```

### Секция: Репо не найдены

Список сервисов для которых не удалось найти репо в Bitbucket —
это потенциальные "сироты" или сервисы с нестандартным именованием.

### Цветовая маркировка строк таблицы

- Зелёная строка: репо найдено, версии совпадают
- Жёлтая строка: репо найдено, версии расходятся  
- Красная строка: репо не найдено
