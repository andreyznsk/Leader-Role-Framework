...
При любых изменениях в репозитории (создание CR, Issue, веток, коммитов, PR) необходимо соблюдать следующий алгоритм.

### 1. Определение базовой ветки

Перед созданием новой ветки НЕ использовать автоматически `master` или `main`.

Сначала определить актуальную рабочую ветку проекта.

Алгоритм:

1. Проверить GitHub и найти последние коммиты пользователя.
2. Определить, в какой ветке находится самый последний коммит пользователя.
3. Если последний коммит принадлежит feature/release/hotfix ветке, считать именно её текущей рабочей веткой.
4. Создавать новую ветку от HEAD этой ветки.
5. Если определить рабочую ветку невозможно (например несколько параллельных веток или неоднозначность), сначала спросить пользователя.
6. Использовать `master` или `main` только если:
    - пользователь явно попросил;
    - либо рабочая feature-ветка отсутствует.

### 2. Создание новой ветки

Новая ветка всегда создаётся от HEAD определённой рабочей ветки.

Никогда не создавать автоматически ветку от `master/main`, если существует более свежая рабочая ветка пользователя.

шаблон feature/{PREFIX}-{NNN}-{YYYY-MM-DD}

### 3. Коммиты

Каждый логически законченный шаг оформлять отдельным commit.

### 4. После создания CR

Обязательно:

- обновить `docs/cr/REGISTRY.md`;
...
Если невозможно однозначно определить рабочую ветку — задать пользователю вопрос.

Не принимать решение самостоятельно.
