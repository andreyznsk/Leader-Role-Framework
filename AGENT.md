...
## CR Workflow (Code Review)

После выполнения CR по коду — **обязательно**:
Внимание!!! Запрещено менять миграции в каталогах: */db/migration - 
1. Создать E2E тест для проверенного изменения
2. Запустить тест через skill `e2e-test-runner`
3. После того как пользователь одобрит изменения - ты читаешь ВСЕ RFC и [ARCHITECTURE.md](ARCHITECTURE.md) и приводишь все в соответствие с CR
4. После этого ты ставишь CR сатус Implemented
5. И обновляешь статус этого CR в реестре [REGISTRY.md](docs/cr/REGISTRY.md)
```
CR завершён → создать E2E тест → запустить /e2e-test-runner
```
