...
3. **Текущие задачи** — список активных задач с фильтрами и сортировкой по полям (col-lg-9)
4. **Календарный сайдбар** — компактный виджет навигации по дедлайнам (col-lg-3, CR-MEM-014)
...
- фильтры: `priority`, `status` (фильтр по `dueDate` удалён из тулбара, перенесён в Calendar sidebar)
- сортировка кнопками `↑/↓` у полей `priority`, `status`
...
**Календарный сайдбар** (CR-MEM-014):

Расположен справа от списка задач (col-lg-3). Единственный инструмент навигации по дедлайнам.

- Компактный месячный грид с кнопками навигации `‹ / ›`
- Цветовые индикаторы на каждом дне по числу задач с таким дедлайном:
  - 🟢 1 задача
  - 🟡 2–3 задачи
  - 🔴 4+ задачи
- Клик по дню — фильтр `?dueDate=YYYY-MM-DD` (повторный клик снимает фильтр)
- Пресеты под гридом: **Today**, **Tomorrow**, **This Week**
  - *This Week* использует параметры `?dueDateFrom=...&dueDateTo=...` (пн–вс текущей недели)
- Кнопка **Reset** — сбрасывает все активные фильтры, возвращает полный список задач
- При наличии активного `dueDate` или `dueDateFrom` — календарь открывается на нужном месяце
- Данные для индикаторов (`deadlineCounts`) вычисляются в `TodayViewController` через `Collectors.groupingBy` по `allCurrentTasks.dueDate`

`TodayViewController` поддерживает три режима фильтрации по дате:

| Параметр | Режим |
|----------|-------|
| `dueDate=YYYY-MM-DD` | Точное совпадение (клик по дню) |
