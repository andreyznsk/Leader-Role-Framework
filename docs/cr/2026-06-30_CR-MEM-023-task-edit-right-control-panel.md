...
- [x] На странице редактирования задачи основная форма находится слева, управление задачей — справа.
- [x] В правом столбце элементы идут строго в порядке: Priority → Deadline → Status → Buttons → Timeline.
- [x] Status реализован как выпадающий список.
- [x] Status dropdown содержит все актуальные возможные статусы задачи.
- [x] Кнопка `Save` сохраняет изменения и оставляет пользователя на странице редактирования.
- [x] Кнопка `Save and close` сохраняет изменения и возвращает пользователя на Today / previous list.
- [x] Кнопка `Archive` архивирует задачу и визуально оформлена как destructive red action.
- [x] Все action buttons имеют иконки и читаемые labels.
- [x] Timeline отображается ниже блока управления в правом столбце.
- [x] Timeline empty state отображается корректно, если событий нет.
- [x] На узком экране layout не ломается и становится одноколоночным.
- [x] Существующий backend API и схема БД не меняются.
- [x] Существующие сценарии task edit и task timeline/archive flow продолжают проходить.
