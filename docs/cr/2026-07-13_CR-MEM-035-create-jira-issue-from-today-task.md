...
- выбор issue type;
- выбор исполнителя: текущий Jira-пользователь, исполнитель LeaderOS-задачи при однозначном сопоставлении, либо без назначения;
- создание Jira issue;
...
- найденного Jira-пользователя для assigned person, если есть;
...
- исполнитель:
  ...
  - исполнитель LeaderOS-задачи, если найдено однозначное соответствие;
  - без назначения.
...
## Сопоставление исполнителя

Для MVP использовать следующий порядок:

1. Если у задачи нет `assignedPerson`, default — текущий Jira-пользователь.
2. Если assigned person есть, попытаться найти assignable Jira user по email.
3. Если найден ровно один пользователь, предложить его в select.
4. Если не найден или результат неоднозначен, не назначать автоматически и показать предупреждение.
5. Никогда не сопоставлять автоматически только по display name.

Постоянная таблица user mapping не входит в MVP.
...
- [ ] Пользователь может выбрать issue type и допустимого исполнителя.
...
- [ ] Добавлен и пройден E2E-сценарий.
- [ ] Существующие Flyway migrations не изменены; добавлена только новая migration.
- [ ] После подтверждения пользователя документация RFC и ARCHITECTURE.md приведена в соответствие, CR переведён в `Implemented`, реестр обновлён.
