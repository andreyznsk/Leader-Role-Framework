...
1. В схемах и тексте местами зашито `Claude AI` / `Claude-агент`, хотя актуальная архитектура уже использует общий `common` модуль и `AgentClient` с провайдерами:
   - `claude`
   - `mock`
   - `ollama`
   - `gigachat`
...
2. Актуализировать scenario flow-схемы, если они напрямую вызывают Claude вместо `AgentClient`.
3. Сделать Mermaid-схемы крупнее.
4. Центрировать Mermaid-схемы по горизонтали и вертикали.
5. Убрать или уменьшить боковые панели, если они мешают крупному отображению диаграммы.
6. Обновить wording: `Claude AI` → `AI Agent` / `LLM Provider` / `AgentClient`, где это архитектурно корректно.
...
- [ ] Убраны устаревшие формулировки `Claude AI` там, где нужен generic `AI Agent` / `LLM Provider`.
- [ ] Scenario-схемы не создают впечатление, что сервисы напрямую завязаны только на Claude.
- [ ] Mermaid-схемы стали крупнее и читаются в fullscreen.
- [ ] Mermaid-схемы расположены по центру экрана.
- [ ] Презентация остаётся standalone static HTML.
- [ ] Нет ошибок Mermaid в browser console.

## UX / Styling требования

1. Mermaid должен занимать большую часть полезной области слайда.
...
3. Если боковая колонка на архитектурном слайде делает схему слишком маленькой — убрать колонку или перенести её в подпись под схемой.
...
3. Проверить:
   ...
   - нет устаревшего текста `Claude AI` в местах, где должен быть generic provider.
